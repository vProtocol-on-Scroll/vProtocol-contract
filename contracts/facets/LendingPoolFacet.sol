...
contract LendingPoolFacet {
    ...
    function _calculateNormalizedDebt(
        address token,
        uint256 amount
    ) internal view returns (uint256) {
        TokenData storage tokenData = s.tokenData[token];
    ...
    function _calculateDepositRate(
        uint256 borrowRate,
        uint256 utilization
    ) internal view returns (uint256) {
        return
            (borrowRate *
                utilization *
                (10000 - s.lendingPoolConfig.reserveFactor)) / (10000 * 10000);
    }
    ...
    function _updateState(address token) internal {
        ...
        uint256 utilization = tokenData.totalDeposits == 0
            ? 0
    ...
    function addSupportedToken(
        ...
        if (s.vaults[token] == address(0)) {
            ...
            string memory vaultName = string(
                abi.encodePacked("vProtocol ", tokenSymbol)
            );
            string memory vaultSymbol = string(
                abi.encodePacked("v", tokenSymbol)
            );
    ...
    function deposit(address token, uint256 amount) external {
        ...
        activity.totalLendingAmount += LibGettersImpl._getUsdValue(
            s,
            token,
            amount,
    ...
    function withdraw(
        ...
        require(
            ...
            "Withdrawal would affect health factor"
        );
    ...
    function borrow(address token, uint256 amount) external {
        ...
        uint256 borrowUsdValue = LibGettersImpl._getUsdValue(
            s,
            token,
            amount,
            decimal
    ...
    function repay(address token, uint256 amount) external returns (uint256) {
        ...
        uint256 normalizedRepayment = _calculateNormalizedDebt(
            token,
            repayAmount
        );
    ...
    //     uint256 newCollateralValueInEth = collateralValueInEth -
    //         withdrawValueInEth;
    ...
    //             newCollateralValueInEth,
    //             currentDebt
    //         );
    //         require(
    //             newHealthFactor >= Constants.HEALTH_FACTOR_THRESHOLD,
    ...
    function liquidate(
        ...
        require(
            s.supportedTokens[collateralToken],
        ...
        if (healthFactor < 5000) {
            ...
            maxLiquidatableDebt =
                (userDebt * Constants.LIQUIDATION_CLOSE_FACTOR_DEFAULT) /
                10000;
        }

        uint256 debtToLiquidate = debtAmount > maxLiquidatableDebt
            ? maxLiquidatableDebt
            : debtAmount;
        ...
        uint256 collateralToLiquidate = (debtToLiquidate *
            debtTokenPrice *
            liquidationBonus) / (collateralPrice * 10000);
        ...
        if (collateralToLiquidate > userCollateral) {
            ...
            debtToLiquidate =
                (collateralToLiquidate * collateralPrice * 10000) /
                (debtTokenPrice * liquidationBonus);
        }

        // Calculate normalized debt to reduce
        uint256 normalizedToReduce = (debtToLiquidate * normalizedDebt) /
            userDebt;
        ...
        UserActivity storage activity = s.userActivities[msg.sender];
        activity.totalLiquidationAmount += _valueInEth(
            debtToken,
            debtToLiquidate
        );
    ...
    function withdrawFromVault(
        ...
        require(
            assets <= s.tokenBalances[token].poolLiquidity,
        ...
        )
    {
        ...
        (ltv, currentLiquidationThreshold) = _calculateUserLtvAndThreshold(
            user
        );

        availableBorrowsETH = totalDebtETH >= (totalCollateralETH * ltv) / 10000
            ? 0
            : ((totalCollateralETH * ltv) / 10000) - totalDebtETH;
    ...
    function _updateRates(address token) internal {
        ...
        if (utilization <= optimalUtilization) {
            ...
            borrowRate =
                (s.lendingPoolConfig.baseRate * utilization) /
                optimalUtilization;
        } else {
            // Exponential increase after optimal utilization
            uint256 excessUtilization = utilization - optimalUtilization;
            uint256 slopeExcess = (excessUtilization *
                s.lendingPoolConfig.slopeExcess) / (10000 - optimalUtilization);
        ...
        uint256 depositRate = (borrowRate *
            utilization *
            (10000 - reserveFactor)) / (10000 * 10000);
     ...
     */
    function _calculateTotalCollateral(
        address user
    ) internal view returns (uint256) {
    ...
    function _calculateTotalDebt(address user) internal view returns (uint256) {
        ...
        for (uint256 i = 0; i < supportedTokens.length; i++) {
            ...
            if (normalizedDebt > 0) {
                uint256 actualDebt = _calculateActualDebt(
                    token,
                    normalizedDebt
                );
     ...
     */
    function _calculateUserLtvAndThreshold(
        address user
        ...
        for (uint256 i = 0; i < supportedCollaterals.length; i++) {
            ...
            if (collateralAmount > 0) {
                uint256 collateralValueETH = _valueInEth(
                    token,
                    collateralAmount
                );
     ...
     */
    function _calculateWeightedLtvAndThreshold()
        internal
        view
