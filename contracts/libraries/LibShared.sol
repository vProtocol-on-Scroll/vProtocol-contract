...
library LibFinance {
    ...
    function calculateHealthFactor(
        ...
        (uint256 totalDebtUSD, uint256 totalCollateralUSD, ) = getUserDebtAndCollateral(s, user);
        ...
        uint256 liquidationThreshold = getWeightedLiquidationThreshold(s, user, totalCollateralUSD);
        
        // Adjusted collateral = collateral * liquidation threshold
        uint256 adjustedCollateral = (totalCollateralUSD * liquidationThreshold) / Constants.BASIS_POINTS;
    ...
    function isLiquidatable(
        ...
        return healthFactor < Constants.HEALTH_FACTOR_THRESHOLD && getUserTotalDebt(s, user) > 0;
    }
    ...
    function calculateInterest(
        ...
        return (principal * rate * timeDelta) / (Constants.BASIS_POINTS * Constants.SECONDS_PER_YEAR);
    }
    ...
    function calculateBorrowRate(
        ...
        if (utilization <= optimalUtilization) {
            ...
            return baseRate + slopeRate + 
                   (excessUtilization * slopeExcess) / (Constants.BASIS_POINTS - optimalUtilization);
        }
    }
    ...
    function calculateDepositRate(
        ...
        return (borrowRate * utilization * (Constants.BASIS_POINTS - reserveFactor)) / 
    ...
    function getUserDebtAndCollateral(
        ...
        for (uint256 i = 0; i < s.s_supportedTokens.length; i++) {
            ...
            if (totalTokenDebt > 0) {
                totalDebtUSD += s.getTokenUsdValue(token, totalTokenDebt, decimal);
            }
            ...
            if (collateralAmount > 0) {
                ...
                uint256 tokenLiqThreshold = s.tokenConfigs[token].liquidationThreshold;
