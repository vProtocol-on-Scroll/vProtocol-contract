...
contract P2pFacetTest is Test {
    ...
    function testCreateLoanListing() public {
        ...
        (uint96 listingId, uint96[] memory matchedRequests) = P2pFacet(payable(address(diamond))).createLoanListingWithMatching(
            1000 * 10**18,  // Amount 
        ...
        LoanListing memory listing = P2pFacet(payable(address(diamond))).getLoanListing(listingId);
            
        assertEq(listing.author, lender, "Author should be the lender");
        assertEq(listing.tokenAddress, address(mockUSDC), "Token should be USDC");
        assertEq(listing.amount, 1000 * 10**18, "Amount should match");
        assertTrue(listing.listingStatus == ListingStatus.OPEN, "Listing should be open");
    }
    ...
    function testCreateLendingRequest() public {
        ...
        (uint96 requestId, bool matched) = P2pFacet(payable(address(diamond))).createAndMatchLendingRequest(
        ...
        Request memory request = P2pFacet(payable(address(diamond))).getRequest(requestId);
        assertEq(request.author, borrower, "Author should be the borrower");
        assertEq(request.amount, 1000 * 10**18, "Amount should match");
        assertEq(request.loanRequestAddr, address(mockUSDC), "Token should be USDC");
        assertEq(uint8(request.status), uint8(Status.OPEN), "Status should be OPEN");
    }
    ...
    function testServiceRequest() public {
        ...
        vm.startPrank(lender);
        mockUSDC.approve(address(diamond), 1000 * 10**18);
        P2pFacet(payable(address(diamond))).serviceRequest(1, address(mockUSDC));
        vm.stopPrank();
        
        // Verify request was serviced
        Request memory request = P2pFacet(payable(address(diamond))).getRequest(1);
        assertEq(uint8(request.status), uint8(Status.SERVICED), "Status should be SERVICED");
    ...
    function testRepayLoan() public {
        ...
        assertEq(uint8(request.status), uint8(Status.CLOSED), "Status should be CLOSED");
    ...
    function testLiquidateUserRequest() public {
        ...
        bool isLiquidatable = P2pFacet(payable(address(diamond))).isPositionLiquidatable(borrower, 1);
        ...
        // Verify liquidation
        Request memory request = P2pFacet(payable(address(diamond))).getRequest(1);
        assertEq(uint8(request.status), uint8(Status.LIQUIDATED), "Status should be LIQUIDATED");
    ...
    function testAutoMatchingLoan() public {
        ...
        (uint96 listingId, ) = P2pFacet(payable(address(diamond))).createLoanListingWithMatching(
            1000 * 10**18,  // Amount 
            100 * 10**18,   // Min amount
            500 * 10**18,   // Max amount
            block.timestamp + 30 days,        // Return duration
        ...
        assertEq(listing.amount, 500 * 10**18, "Listing amount should be reduced");
        assertTrue(listing.listingStatus == ListingStatus.OPEN, "Listing should still be open with remaining amount");
    }
    ...
    function testUseExistingCollateral() public {
        ...
        LendingPoolFacet(payable(address(diamond))).deposit(collateralTokens[0], collateralAmounts[0], true);
        vm.stopPrank();
        ...
        assertEq(request.collateralTokens.length, 1, "Should use 1 collateral token");
        assertEq(request.collateralTokens[0], address(mockWETH), "Should use WETH collateral");
    }
