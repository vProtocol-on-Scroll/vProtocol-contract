...
contract LendingPoolFacetTest is Test {
    ...
    function setUp() public {
        ...
        Diamond(payable(address(diamond))).initialize(tokens, priceFeeds, address(2));
    ...
    function testDepositToLendingPool() public {
        ...
        uint256 shares = LendingPoolFacet(payable(address(diamond))).deposit(
            address(mockUSDC),
            1000 * 10**18,
            true // as collateral
        );
        ...
        // Get user position
        vm.startPrank(user1);
        uint256 collateral = LendingPoolFacet(payable(address(diamond))).getUserTokenCollateral(user1, address(mockUSDC));
        vm.stopPrank();
    ...
    function testBorrowFromLendingPool() public {
        ...
        uint256 loanId = LendingPoolFacet(payable(address(diamond))).createPosition(
            collateralTokens,
            collateralAmounts,
            address(mockUSDC),
        ...
        // Verify loan details
        (PoolLoanDetails memory loanDetails, , uint256 healthFactor) = 
        ...
        assertEq(loanDetails.borrowToken, address(mockUSDC), "Borrowed token should be USDC");
        assertEq(loanDetails.borrowAmount, 7500 * 10**6, "Borrowed amount should match");
        assertGt(healthFactor, 10000, "Health factor should be healthy (>100%)");
    }
    ...
    function testRepayLoan() public {
        ...
        uint256 amountRepaid = LendingPoolFacet(payable(address(diamond))).repay(loanId, 2000 * 10**6);
        vm.stopPrank();
        ...
        (PoolLoanDetails memory loanDetails, uint256 currentDebt, uint256 healthFactor) = 
            LendingPoolFacet(payable(address(diamond))).getLoanDetails(loanId);
        
        assertEq(uint8(loanDetails.status), uint8(LoanStatus.ACTIVE), "Loan should still be active");
        assertEq(loanDetails.borrowAmount, 5500 * 10**6, "Remaining amount should be reduced");
        
        // Wait for interest to accrue
        vm.warp(block.timestamp + 1000000000000000000);

        (loanDetails, currentDebt, healthFactor) = 
            LendingPoolFacet(payable(address(diamond))).getLoanDetails(loanId);

        // Now repay the full loan
        vm.startPrank(user1);
        mockUSDC.approve(address(diamond), currentDebt);
        amountRepaid = LendingPoolFacet(payable(address(diamond))).repay(loanId, 0); // 0 means full repayment
        vm.stopPrank();
        
        // Verify full repayment
        assertGt(amountRepaid, 5500 * 10**6, "Full repayment should include interest");
        
        // Loan should now be repaid
        (loanDetails, currentDebt, healthFactor) = 
            LendingPoolFacet(payable(address(diamond))).getLoanDetails(loanId);
            
        assertEq(uint8(loanDetails.status), uint8(LoanStatus.REPAID), "Loan should be marked as repaid");
    }
    ...
    function testLiquidation() public {
        ...
        bool isLiquidatable = LendingPoolFacet(payable(address(diamond))).isLoanLiquidatable(loanId);
        assertTrue(isLiquidatable, "Loan should be liquidatable after price drop");
        
        // User2 liquidates the loan
        vm.startPrank(user2);
        mockUSDC.approve(address(diamond), 8000 * 10**18); // Enough to cover full loan
        uint256 collateralReceived = LendingPoolFacet(payable(address(diamond))).liquidateLoan(loanId);
        vm.stopPrank();
        
        // Verify liquidation
        assertGt(collateralReceived, 0, "Liquidator should receive collateral");
        assertGt(mockWETH.balanceOf(user2), INITIAL_BALANCE, "Liquidator should have received ETH collateral");
        
        // Loan should now be liquidated
        (PoolLoanDetails memory loanDetails, , ) = 
            LendingPoolFacet(payable(address(diamond))).getLoanDetails(loanId);
            
        assertEq(uint8(loanDetails.status), uint8(LoanStatus.LIQUIDATED), "Loan should be marked as liquidated");
    }
    ...
    function testWithdraw() public {
        ...
        // Verify withdrawal
        assertEq(amountWithdrawn, 500 * 10**18, "Withdrawn amount should match");
