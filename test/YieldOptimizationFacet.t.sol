...
contract YieldOptimizationTest is Test {
    ...
    function setUp() public {
        ...
        diamond.initialize(address(protocolToken));
        
        // Initialize the yield system
        YieldOptimizationFacet(payable(address(diamond))).initializeYieldSystem(address(rewardToken));
    ...
    function test_InitializeYieldSystem() public {
        ...
        YieldOptimizationFacet(payable(address(newDiamond))).initializeYieldSystem(address(rewardToken));
    ...
    function test_StakeTokens() public {
        ...
        uint256 expectedMultiplier = BASIS_POINTS + (lockDuration * BASIS_POINTS) / MAX_LOCK_PERIOD;
        ...
        // Stake tokens
        YieldOptimizationFacet(payable(address(diamond))).stake(stakeAmount, lockDuration, true);
        ...
        ) = YieldOptimizationFacet(payable(address(diamond))).getUserStakeInfo(user1);
        
        // Verify stake info
        assertEq(amount, stakeAmount, "Stake amount incorrect");
        assertEq(lockEnd, block.timestamp + lockDuration, "Lock end incorrect");
        assertEq(loyaltyMultiplier, expectedMultiplier, "Loyalty multiplier incorrect");
    ...
    function test_StakeTokensInvalidDuration() public {
        ...
        YieldOptimizationFacet(payable(address(diamond))).stake(stakeAmount, tooShortDuration, true);
        
        // Test too long duration
        uint256 tooLongDuration = 366 days;
        vm.expectRevert("Invalid duration");
        YieldOptimizationFacet(payable(address(diamond) )).stake(stakeAmount, tooLongDuration, true);
    ...
    function test_UnstakeTokens() public {
        ...
        protocolToken.approve(address(diamond), stakeAmount);
        YieldOptimizationFacet(payable(address(diamond))).stake(stakeAmount, lockDuration, false);
        ...
        // balance of protocol
        uint256 protocolTokenBalance = protocolToken.balanceOf(address(diamond));
        ...
        assertEq(balanceAfter, balanceBefore + stakeAmount, "Unstaked amount not received");
        
        // Verify stake is cleared
        (uint256 amount, , , ) = YieldOptimizationFacet(payable(address(diamond))).getUserStakeInfo(user1);
    ...
    function test_UpdateStrategy() public {
        ...
        // Update strategy
        YieldOptimizationFacet(payable(address(diamond))).updateStrategy(strategyId, allocationWeights);
    ...
    function test_InvalidStrategyUpdate() public {
        ...
        YieldOptimizationFacet(payable(address(diamond))).updateStrategy(strategyId, invalidWeights);
    ...
    function test_RewardAccrual() public {
        ...
        (, , , uint256 pendingRewards) = YieldOptimizationFacet(payable(address(diamond))).getUserStakeInfo(user1);
        ...
        assertEq(rewardBalanceAfter, rewardBalanceBefore + pendingRewards, "Rewards not transferred correctly");
        
        // Check that pending rewards are reset
        (, , , pendingRewards) = YieldOptimizationFacet(payable(address(diamond))).getUserStakeInfo(user1);
    ...
    function test_AutoCompounding() public {
        ...
        (, , , uint256 pendingRewardsBefore) = YieldOptimizationFacet(payable(address(diamond))).getUserStakeInfo(user2);
        ...
        YieldOptimizationFacet(payable(address(diamond))).executeAutoCompounding();
        
        // Check stake amount after compounding
        (uint256 newStakeAmount, , , uint256 pendingRewardsAfter) = YieldOptimizationFacet(payable(address(diamond) )).getUserStakeInfo(user2);
        
        // Verify stake increased and rewards were reset
        assertTrue(newStakeAmount > stakeAmount, "Stake not increased after compounding");
        assertEq(pendingRewardsAfter, 0, "Rewards not reset after compounding");
        assertEq(newStakeAmount, stakeAmount + pendingRewardsBefore, "Compounded amount incorrect");
    ...
    function test_MultipleUsers_RewardDistribution() public {
        ...
        YieldOptimizationFacet(payable(address(diamond))).stake(stakeAmount1, lockDuration1, false);
        vm.stopPrank();
        ...
        YieldOptimizationFacet(payable(address(diamond))).stake(stakeAmount2, lockDuration2, true);
        vm.stopPrank();
        ...
        YieldOptimizationFacet(payable(address(diamond))).stake(stakeAmount3, lockDuration3, false);
        vm.stopPrank();
        ...
        (, , , uint256 rewards1) = YieldOptimizationFacet(payable(address(diamond))).getUserStakeInfo(user1);
        (, , , uint256 rewards2) = YieldOptimizationFacet(payable(address(diamond))).getUserStakeInfo(user2);
        (, , , uint256 rewards3) = YieldOptimizationFacet(payable(address(diamond))).getUserStakeInfo(user3);
        ...
        assertTrue(rewards2 > rewards3, "User with longer lock should have higher rewards");
        
        // User 1 should have higher rewards than User 3 due to larger stake
        assertTrue(rewards1 > rewards3, "User with larger stake should have higher rewards");
    ...
    function test_LoyaltyMultiplierEffects() public {
        ...
        YieldOptimizationFacet(payable(address(diamond))).stake(stakeAmount, 7 days, false);
        
        // Stake with medium duration (180 days)
        YieldOptimizationFacet(payable(address(diamond))).stake(stakeAmount, 180 days, false);
        
        // Stake with maximum duration (365 days)
        YieldOptimizationFacet(payable(address(diamond))).stake(stakeAmount, 365 days, false);
        
        // Fast forward 30 days
        vm.warp(block.timestamp + 30 days);
        
        // Calculate expected multipliers
        uint256 shortMultiplier = BASIS_POINTS + (7 days * BASIS_POINTS) / MAX_LOCK_PERIOD;
        uint256 mediumMultiplier = BASIS_POINTS + (180 days * BASIS_POINTS) / MAX_LOCK_PERIOD;
        uint256 maxMultiplier = BASIS_POINTS + (365 days * BASIS_POINTS) / MAX_LOCK_PERIOD;
        
        // Expected multipliers: ~10% for 7 days, ~50% for 180 days, 100% for 365 days
        assertTrue(mediumMultiplier > shortMultiplier, "Medium multiplier should be higher than short");
        assertTrue(maxMultiplier > mediumMultiplier, "Max multiplier should be higher than medium");
        assertEq(maxMultiplier, BASIS_POINTS * 2, "Max multiplier should be 2x");
        
        vm.stopPrank();
    }

    // Helper function to get function selectors from a facet
    function getFunctionSelectors(string memory _facetName) internal returns (bytes4[] memory selectors) {
        if (keccak256(abi.encodePacked(_facetName)) == keccak256(abi.encodePacked("DiamondLoupeFacet"))) {
        ...
        } else if (keccak256(abi.encodePacked(_facetName)) == keccak256(abi.encodePacked("YieldOptimizationFacet"))) {
            selectors = new bytes4[](9);
            selectors[0] = YieldOptimizationFacet.initializeYieldSystem.selector;
            ...
            selectors[5] = YieldOptimizationFacet.executeAutoCompounding.selector;
